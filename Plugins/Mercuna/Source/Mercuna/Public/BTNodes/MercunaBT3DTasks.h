...
#include "MercunaBT3DTasks.generated.h"

class UMercuna3DTask_MoveTo;
class UBlackboardComponent;
struct FMercunaMoveRequest3D;

struct FBTMercuna3DMoveToTaskMemory
{
    FDelegateHandle BBObserverDelegateHandle;
    FVector PreviousGoalLocation;

    TWeakObjectPtr<UMercuna3DTask_MoveTo> Task;

    uint8 bObserverCanFinishTask : 1;
};

/**
* Mercuna 3D Move To task node.
* Moves the AI pawn toward the specified Actor or Location blackboard entry using Mercuna 3D navigation
*/
UCLASS(meta = (DisplayName = "Mercuna 3D Move To"))
class MERCUNA_API UBTTask_Mercuna3D_MoveTo : public UBTTask_BlackboardBase
{
    ...
    /** If the target is an obstacle then should avoidance be run be against it */
    UPROPERTY(Category = Node, EditAnywhere, AdvancedDisplay)
...
protected:
    ...
    virtual UMercuna3DTask_MoveTo* PrepareMoveTask(UBehaviorTreeComponent& OwnerComp, UMercuna3DTask_MoveTo* ExistingTask, FMercunaMoveRequest3D& MoveRequest);
};
